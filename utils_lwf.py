...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, normalize='none', get_d_thetas=False, grad=None, it_eval=None, net_copy=None):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        #     loss = criterion(output, lab, weight)
        # else:
        #     loss = criterion(output, lab)
...
def train_lwf(images_train, labels_train,cur_img_syn,cur_lbl_syn, net, optimizer, criterion, args, aug, normalize='none', get_d_thetas=False, grad=None, it_eval=None, net_copy=None):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.cuda()
    criterion = criterion.cuda()
    net.train()
    # images_train = cur_img_syn.cuda()
    # labels_train = cur_lbl_syn.cuda()
    ...
    for i_batch, datum in enumerate(trainloader):
        ...
        if it_eval > 0: 
            # 旧moxing
            output_old = net_copy(img)
            dist_loss = MultiClassCrossEntropy(output_old, output, 2) # 历史监督损失(old,new,T), T为温度常数
            loss = loss + 0.01* dist_loss
        # loss = cls_loss
        if len(lab.shape) > 1: # one
            acc = np.sum(np.equal(np.argmax(output.cpu().data.numpy(), axis=-1), lab.argmax(dim=-1).cpu().numpy()))
        else:
...
def evaluate_synset(it_eval, net, images_train, labels_train,cur_img_syn,cur_lbl_syn ,testloader, args, lr=None, verbose=True, fast=False, weight=None):
    ...
    for ep in range(Epoch+1):
        ...
        # loss_train, acc_train, _ = train_lwf( images_train, labels_train,cur_img_syn,cur_lbl_syn, net, optimizer, criterion, args, aug=(not args.no_aug), normalize=args.normalize_input, it_eval=it_eval, net_copy=net_copy)
        lr_schedule(ep)
...
def evaluate_synset2(it_eval, net,  images_train, labels_train, args, verbose=True):
    """ evaluate a TRAINED model on fullset """
    # if loader is None:
    #     return -99
    images_train = images_train.cuda()
    ...
    # dst_train = torch.utils.data.TensorDataset(images_train, labels_train)
    loader = torch.utils.data.DataLoader(dst_train, batch_size=args.batch_train, shuffle=True, num_workers=0)
...
def train_synset(it_eval,net,images_train,labels_train,args,lr=None):
## get model and optimizer
    net = net.cuda()
    net_copy = copy.deepcopy(net)
    lr = float(args.lr_net) if lr == None else lr
